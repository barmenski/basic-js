...
encrypt(m, k){
	...
	for(let i=0, j=0; i<wordMaxlength; i++, j++){ 			
		let mi = abc.indexOf( message[ ( (i>=message.length) ? i%message.length : i ) ] );	//определяем индекс буквы из сообщения В АЛФАВИТЕ с учетом подгона длины сообщения - к ключу (если меньше, но обычно нет)
		...
		if(abc.indexOf(message[ ( (i>=message.length) ? i%message.length : i ) ])===-1){
